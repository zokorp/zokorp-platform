...
export function CaseStudyAccordion({
  ...
  useEffect(() => {
    ...
    const openFromHash = () => {
      ...
      const target = container.querySelector<HTMLDetailsElement>(
        `details[data-slug="${cssEscape(hash)}"]`,
      );
      if (!target) return;
  ...
  const handleToggle = (slug: string) => (event: React.SyntheticEvent<HTMLDetailsElement>) => {
  ...
  return (
    <div
      ref={containerRef}
      className="divide-y divide-border/70 overflow-hidden rounded-[calc(var(--radius-panel-lg)-0.1rem)] border border-border bg-gradient-to-b from-white/90 to-[rgba(245,249,253,0.96)] shadow-[var(--shadow-card)]"
    >
      {studies.map((study, index) => (
        <details
          ...
          id={study.slug}
          name="case-study"
          className="group scroll-mt-28 open:bg-[rgba(250,252,255,0.65)]"
          onToggle={handleToggle(study.slug)}
        >
          <summary className="cursor-pointer list-none px-5 py-6 md:px-8 md:py-7">
            <div className="grid gap-5 md:grid-cols-[6ch_minmax(0,1fr)_auto] md:items-center md:gap-8">
              ...
              <div className="space-y-2">
                <h3 className="font-display max-w-[52ch] text-[1.35rem] font-semibold leading-[1.15] tracking-[-0.02em] text-card-foreground transition group-hover:text-brand md:text-[1.55rem]">
                  {study.title}
                </h3>
                <p className="max-w-[70ch] text-sm leading-7 text-muted-foreground">
                  {study.summary}
                </p>
              ...
              <span
                aria-hidden
                className="inline-flex size-10 items-center justify-center rounded-full border border-border bg-white text-lg font-semibold text-muted-foreground transition group-open:rotate-45 group-open:border-brand group-open:text-brand"
              >
                +
              </span>
            </div>
