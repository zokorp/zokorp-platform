...
// Returns true when the calling route should respond 200 OK without further work
// (used for the "duplicate fulfillment" short-circuit on checkout.session events).
type DispatchOutcome = "done" | "duplicate_short_circuit";

async function handleCheckoutSession(event: Stripe.Event): Promise<DispatchOutcome> {
  ...
  try {
    ...
    if (isDuplicateCheckoutFulfillmentError(error)) {
      ...
      return "duplicate_short_circuit";
    }
...
async function handleCheckoutAsyncPaymentFailed(event: Stripe.Event): Promise<void> {
  const session = event.data.object as Stripe.Checkout.Session;
...
async function handleChargeRefunded(event: Stripe.Event): Promise<void> {
  ...
  const stripeInvoiceId = getChargeInvoiceId(charge);
...
async function handleUnsupportedEvent(event: Stripe.Event): Promise<void> {
  await recordStripeWebhookEvent({
    event,
    processingStatus: "ignored",
    metadata: {
...
// Dispatch a verified Stripe event to its handler. Returns "duplicate_short_circuit"
// when the caller should immediately respond 200 OK without further processing
// (e.g. duplicate Checkout Session delivery already fulfilled).
export async function dispatchStripeWebhookEvent(event: Stripe.Event): Promise<DispatchOutcome> {
  switch (event.type) {
    ...
    default:
      await handleUnsupportedEvent(event);
      return "done";
  }
}
