...
| Direct demos / invite-only soft launch to `app.zokorp.com` | GO WITH CAVEATS | `/services`, admin hardening, billing resilience, and security fixes are deployed, but several live browser-only proofs remain open |
...
| Broad promotion of paid validator flow | NO-GO UNTIL ONE STRIPE TEST CHECKOUT | The server-side path is strong, but one fresh browser checkout should be completed before public promotion |
...
- `app.zokorp.com` production deployment is healthy after deploy `dpl_DhvHvU1EAc84o5UHpyKEVgKVeK5d`
- `npm run lint` passed
...
- one live Stripe browser checkout is still missing
...
3. Run one Atlas/browser auth pass using:
   ...
   - `zkhawaja@zokorp.com`
4. Run one Stripe test-mode browser checkout and confirm fulfillment.
...
4. Do not claim fully proven paid checkout or fully proven booked-call automation until the remaining browser/manual proofs are completed.
