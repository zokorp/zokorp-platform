...
- [TODO][AUTO] CRIT-003 Implement browser-side SVG text and dimension extraction for the Architecture Diagram Reviewer. Done when: SVG parsing runs in the browser and produces the same or better evidence than the current server path.
...
- [TODO][AUTO] CRIT-007 Rework the Architecture Diagram Reviewer progress UX to reflect browser preprocessing plus server scoring instead of a fake local-model phase. Done when: all visible phases map to real work and the “local model refinement” copy is either implemented or removed.
- [TODO][AUTO] CRIT-008 Standardize a single verified-business-email access strategy for all free tools. Done when: each free tool follows the same rule for account creation, verification, and result delivery.
- [TODO][AUTO] CRIT-009 Build a shared free-tool access gate component that explains why business-email verification is required and reuses account context consistently. Done when: Architecture Review, AI Decider, Landing Zone, and Cloud Cost use the same gate.
- [TODO][AUTO] CRIT-010 Ensure no free tool sends consulting output to an unverified account. Done when: verified-business-email checks are enforced server-side for every quote-delivery path.
...
- [TODO][AUTO] CRIT-025 Add a static fallback catalog for public software and pricing pages when the database catalog is unavailable. Done when: the site still shows core public offerings even if the DB is down or not configured.
