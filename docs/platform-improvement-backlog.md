...
- [DONE][AUTO] CRIT-004 Change the Architecture Diagram Reviewer submit flow so the server receives structured extracted evidence rather than the raw uploaded diagram. Done when: the submit API no longer requires raw diagram bytes for normal review processing.
  - Evidence: `app/api/submit-architecture-review/route.ts` now validates file bytes only during the request, requires browser-extracted PNG/SVG evidence before queuing, optionally archives the original diagram immediately when follow-up archival is requested, and `lib/architecture-review/jobs.ts` builds the review solely from structured evidence stored in metadata.
- [DONE][AUTO] CRIT-005 Remove `diagramBytes` persistence from the architecture-review job model and add the required Prisma migration. Done when: raw diagram bytes are no longer stored in Postgres and the schema, code, and tests are updated.
  - Evidence: Removed `diagramBytes` from `prisma/schema.prisma`, added migration `0011_architecture_review_remove_raw_bytes`, updated job creation/processing to store only metadata plus WorkDrive archive IDs/status, and extended `tests/architecture-review-route.test.ts` to assert the queued job payload no longer contains raw diagram bytes.
- [DONE][AUTO] CRIT-006 Remove server-side `tesseract.js` from the architecture-review processing path once the browser path is complete. Done when: the review job can run end to end without server OCR for the supported file types.
  - Evidence: `lib/architecture-review/server.ts` now contains only SVG safety/extraction helpers, the submit route rejects uploads that lack browser-extracted evidence, and the architecture-review route/form/schema tests cover the supported PNG/SVG evidence path without any server OCR fallback.
...
- [DONE][AUTO] CRIT-011 Create a shared deterministic quote line-item schema for all diagnostic tools. Done when: every tool quote can render labeled line items, low/high amounts, and reasons from one typed contract.
  - Evidence: Added `lib/quote-line-items.ts` as the shared contract/helper module, switched Cloud Cost to the shared schema, and added `tests/quote-line-items.test.ts` so scaling and deduplication are regression-protected.
- [DONE][AUTO] CRIT-012 Add itemized quote generation to AI Decider using the shared quote schema. Done when: the email and stored quote include line items instead of a range-only summary.
  - Evidence: `lib/ai-decider/quote.ts` now emits shared line items, `lib/ai-decider/types.ts` stores them in the quote schema, and `lib/ai-decider/email.ts` renders the breakdown consistently.
- [DONE][AUTO] CRIT-013 Add itemized quote generation to Landing Zone Readiness using the shared quote schema. Done when: the email and stored quote include deterministic line items instead of a range-only summary.
  - Evidence: `lib/landing-zone-readiness/quote.ts` now emits scaled category/scope line items, `lib/landing-zone-readiness/types.ts` stores them, and `lib/landing-zone-readiness/email.ts` renders the breakdown in both text and HTML results.
- [DONE][AUTO] CRIT-014 Standardize Architecture Diagram Reviewer quote packaging onto the same shared quote schema. Done when: architecture-review quote output, storage, and email rendering align with the other tools.
  - Evidence: `lib/architecture-review/report.ts` now stores `consultationQuote` with shared line items/rationale alongside the existing compatibility fields, and `lib/architecture-review/email.ts` renders the same breakdown pattern used by the other diagnostics.
...
- [DONE][AUTO] MED-008 Add unit tests for shared quote line-item generation. Done when: quote calculations have regression protection across tools.
  - Evidence: Added `tests/quote-line-items.test.ts`, plus shared-schema/email regression coverage in `tests/ai-decider-email.test.ts`, `tests/landing-zone-readiness-email.test.ts`, `tests/landing-zone-readiness-engine.test.ts`, `tests/architecture-review-email.test.ts`, and `tests/architecture-review-schema.test.ts`.
